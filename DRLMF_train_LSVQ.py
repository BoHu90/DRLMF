...
def save_checkpoint(model, optimizer, scheduler, epoch, old_save_name, path='/data/user/XXX/DRLMF/ckpts/DRLMF.pth'):
    """保存模型的checkpoint"""
    state = {
...
def load_checkpoint(model, optimizer, scheduler, path='/data/user/XXX/DRLMF/ckpts/DRLMF.pth'):
    """加载模型的checkpoint"""
...
def main(config):
    ...
    CLIP = modular.ViTbCLIP_SpatialTemporal_modular_dropout(feat_len=10, sr=True, tr=True, dropout_sp=0.2, dropout_tp=0.2).float()
    ...
    start_epoch, old_save_name = load_checkpoint(model, optimizer, scheduler,
                                                 path='/data/user/gbb/SimpleVQA-main/ckpts/DRSMF.pth')


    videos_dir = '/data/user/xxx/DRLMF/data/LSVQ_image_all_fps1_motion'  #frame_path
    frame_dir = '/data/user/xxx/DRLMF/LSVQ_image_all_fps1_motion_4kuai'  #four_split_path
    feature_dir = '/data/user/xxx/DRLMF/VideoMAEv2-master/save_feat/LSVQ_VideoMAE_feat'
    datainfo_train = '/data/user/XXX/SimpleVQA-main/data/LSVQ_whole_train.csv'
    datainfo_test = '/data/user/XXX/SimpleVQA-main/data/LSVQ_whole_test.csv'
    datainfo_test_1080p = '/data/user/XXX/SimpleVQA-main/data/LSVQ_whole_test_1080p.csv'
    ...
    trainset = wide_VideoDataset_images_with_motion_features_and_deformation(videos_dir, frame_dir, feature_dir, datainfo_train,
                                                             ...
                                                             'LSVQ_train', config.crop_size, 'tad_lsvq')
    testset =wide_VideoDataset_images_with_motion_features_and_deformation(videos_dir, frame_dir, feature_dir, datainfo_test,
                                                            ...
                                                            'LSVQ_test', config.crop_size, 'tad_lsvq')
    testset_1080p = wide_VideoDataset_images_with_motion_features_and_deformation(videos_dir, frame_dir, feature_dir,
                                                                            datainfo_test_1080p,
                                                                            transformations_test,
                                                                            transformations_frame_test,
                                                                            'LSVQ_test_1080p', config.crop_size, 'tad_lsvq')
    ...
    test_loader_1080Pp = torch.utils.data.DataLoader(testset_1080p, batch_size=1,
                                              shuffle=False, num_workers=config.num_workers)
    ...
    for epoch in range(start_epoch, config.epochs):  # 10
        ...
        with torch.no_grad():    #下文中不会计算梯度
            ...
            for i, (video, feature_3D, mos, video_name, center_video, block_video1, block_video2, block_video3,
                    block_video4) in enumerate(test_loader_1080Pp):
                video = video.to(device)
                feature_3D = feature_3D.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='DRLMF')  # UGC_BVQA_model
